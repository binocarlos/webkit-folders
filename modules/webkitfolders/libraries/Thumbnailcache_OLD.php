...
class Thumbnailcache
{
	...
	public function process_image($original_image_path, $arguments)
	{
		...
		if(!empty($arguments['photoprint']))
		{
			$depth = escapeshellcmd($arguments['photoprint']);
			...
			$photoprint_args = "-bordercolor '#ffffff' -border 0 -background  none  -rotate ".rand(-10, 10)." -background  black  \( +clone -shadow 60x5+{$depth}+{$depth} \) +swap -background '#{$bgcol}'  -flatten -resize {$imagewidth}x\>";
			
			exec("$convert_path $escaped_original_image_path $photoprint_args -quality $quality $escaped_destination_filename");
